...
from flask_utils import (get_free_gpus, append_dict_to_jsonl, get_end_time, get_start_time, parse_params,
                         ...
                         set_gpu, copy_file, set_all_gpus, upload_file, generate_presigned_url)
...
def run_evaluate():
    ...
    print(len(params.get('model_names')), params_config["model_ids"])
    ...
    if num_gpus_total // num_gpus_per_model > 1:
        import ray
        ray.init(
                    _temp_dir='/home/Userlist/yanganwen',  # 指定新的对象存储路径
            ...
            for item_dict in score:
                for key in item_dict.keys():
                    if start_time in key:
                        with open(f"/home/Userlist/yanganwen/temp/obs/{key}.json", "w", encoding="utf-8") as f:
                        ...
                        scores_out.append({key: {"total_correct": item_dict[key]["total_correct"],
                                           ...
                                           "error_examples_file": gen_eval_report(f"{key}.json")
                                           })
